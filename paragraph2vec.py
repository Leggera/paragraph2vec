...
def word2vec_sgd_wrapper(word2vecModel, tokens, wordVectors, dataset, it, nParagraphs, word2vecCostAndGradient = negSamplingCostAndGradient):   
    prev_idx = []
    batchsize = 50
    ...
    for i in it:
        ...
        idx_in, gin, gin_target, idx_out, gout, gout_target = word2vecModel(p_id, context, tokens, inputVectors, outputVectors, prev_idx, dataset, word2vecCostAndGradient)
        count += 1
        if (count < batchsize):
